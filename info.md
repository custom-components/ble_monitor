...
|**K9B**|**Linptech Switch**<br /><br />Broadcasts the press type (`short press`, `double press`, `long press`) for each button. There are three different versions of this switch, with one, two or three buttons. The switch sensor state will return to `no press` after the time set with the [reset_timer](#reset_timer) option. It is advised to change the reset time to 1 second (default = 35 seconds). Advertisements are probably encrypted (not confirmed yet), you need to set the encryption key in your configuration, see for instructions the [encryption_key](#encryption_key) option.||
